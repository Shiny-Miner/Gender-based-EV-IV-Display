/*Base original del "main_eviv.c" hecho por Acimut*/
...
const u16 gBgEvIvPal_G[] = INCBIN_U16("graphics/Pal_G.gbapal");
const u16 gBgEvIvPal_BS[] = INCBIN_U16("graphics/bgEvIv_Sh.gbapal");
const u16 gBgEvIvPal_GS[] = INCBIN_U16("graphics/Pal_G_Sh.gbapal");
...
#define gHiddenMove         gEvIv->hiddenmove
...
static void Task_WaitForExit(u8 taskId)
{
    ...
    case 1:
        ...
        {
            PlayCry7(&gPlayerParty[gCurrentMon], 0);
        }
...
//Use in windows with Type Icons
//Usado en ventanas con Iconos de Tipos
const u8 gNone[]           = _("");

//Si quieres hacer que aparezcan todas en Mayúsculas deberás escribirlos xd
#ifdef ESP
...
#else
// ENG
//If you want to make all them appear in uppercase you should write them xd
...
static void PrintWindow2(u16 species, u8 isEgg, u8 friendship)
{
    ...
    if (gHiddenMove >= TYPE_MYSTERY){
        ...
        AddTextPrinterParameterized3(WIN_BOTTOM_BOX, 2, BS_X+1, 1, gBlackTextColor, 0, gStringVar1);
        AddTextPrinterParameterized3(WIN_BOTTOM_BOX, 2, EV_X+1, 1, gBlackTextColor, 0, gStringVar2);
    ...
    //Change the color of the background depending on the gender of the protagonist and color the box if the Pokémon is shiny
    //Cambia el color del fondo dependiendo el genero del prota y colorea el recuadro si el Pokémon es shiny
    if (gSaveBlock2Ptr->playerGender == MALE)
        {   
            if(gVarShiny==0){
            LoadPalette(gBgEvIvPal_B, 0, 0x20);
            }else if(gVarShiny==1){
            LoadPalette(gBgEvIvPal_BS, 0, 0x20);
            }
        }else{
            if(gVarShiny==0){
            LoadPalette(gBgEvIvPal_G, 0, 0x20);
            }else if(gVarShiny==1){
            LoadPalette(gBgEvIvPal_GS, 0, 0x20);
            }
        }
    ...
    else
    {
        ...
        AddTextPrinterParameterized3(WIN_BOTTOM_BOX, 2, 6, 17, gBlackTextColor, 0, gStringVar4);
        AddTextPrinterParameterized3(WIN_HIDDEN_MOVE, 2, 3, 2, gBlackTextColor, 0, gNone);
    }
}

static void PrintWindow3(void)
{
        //this is added so that the icons appear... Idk why, but only with this they appear xd
        //se agrega esto para que aparezcan los iconos... no c por que, pero solo con esto aparecen xd
        AddTextPrinterParameterized3(WIN_TYPE, 2, 1, 1, gGrayTextColor, 0, gNone);
}
...
static void PrintWindow4(void)
{
        //this is added so that the icons appear... Idk why, but only with this they appear xd
        //se agrega esto para que aparezcan los iconos... no c por que, pero solo con esto aparecen xd
        AddTextPrinterParameterized3(WIN_HIDDEN_MOVE, 2, 1, 1, gGrayTextColor, 0, gNone);
}
